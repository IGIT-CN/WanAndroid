...
import com.zhuzichu.android.wan.ui.search.domain.UseCaseDropHistoryKey
import com.zhuzichu.android.wan.ui.search.domain.UseCaseGetHotKey
import com.zhuzichu.android.wan.ui.search.domain.UseCaseGetHistoryKey
import com.zhuzichu.android.wan.ui.search.domain.UseCasePlusHistoryKey
...
class ViewModelSearch @Inject constructor(
    ...
    val items: ObservableList<Any> = MergeObservableList<Any>()
        .insertItem(ItemViewModelSearchKeytype(this, "历史记录", itemsHistory, true))
        .insertList(itemsHistory)
        .insertItem(ItemViewModelSearchKeytype(this, "热搜记录", itemsHot, false))
        .insertList(itemsHot)
    ...
    val onSearchCommand = createCommand {
        keyword.value.toString().toast()
        plusHistoryKeyword(keyword.value)
    }
            ...
            .subscribe(
                {
                    itemsHot.update(it.data?.map { item ->
                        ItemViewModelSearchKeyword(this, item.name ?: "", false)
                    })
                }, {
                    handleThrowable(it)
                }
            ...
            .subscribe(
                {
                    itemsHistory.update(it.map { item ->
                        ItemViewModelSearchKeyword(this, item.keyword, true)
                    })
                },
                {
                    handleThrowable(it)
